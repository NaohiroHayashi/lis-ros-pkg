...
{ 
  ...
  return true;
    ...
    geometry_msgs::Quaternion move_rotation(geometry_msgs::Pose pose, double deg){
        geometry_msgs::Quaternion q, returnq;
        q = pose.orientation;
        cv::Mat WRH = cv::Mat::zeros(3,3,CV_64FC1);
        cv::Mat HRCy = cv::Mat::zeros(3,3,CV_64FC1);
        cv::Mat HRCz =cv::Mat::zeros(3,3,CV_64FC1);
        cv::Mat WRC = cv::Mat::zeros(3,3,CV_64FC1);
        cv::Mat CRW = cv::Mat::zeros(3,3,CV_64FC1);

        WRH.at<double>(0,0) = 1.0 - 2.0 * (q.y * q.y + q.z * q.z);
        WRH.at<double>(0,1) =       2.0 * (q.x * q.y - q.w * q.z);
        WRH.at<double>(0,2) =       2.0 * (q.w * q.y + q.x * q.z);
        
        WRH.at<double>(1,0) =       2.0 * (q.x * q.y + q.w * q.z);
        WRH.at<double>(1,1) = 1.0 - 2.0 * (q.x * q.x + q.z * q.z);
        WRH.at<double>(1,2) =       2.0 * (q.y * q.z - q.w * q.x);
        
        WRH.at<double>(2,0) =       2.0 * (q.x * q.z - q.w * q.y);
        WRH.at<double>(2,1) =       2.0 * (q.y * q.z + q.w * q.x);
        WRH.at<double>(2,2) = 1.0 - 2.0 * (q.x * q.x + q.y * q.y);
        
        HRCy.at<double>(0,0) = cos(deg*M_PI/180.0);
        HRCy.at<double>(0,1) = 0.0;
        HRCy.at<double>(0,2) = sin(deg*M_PI/180.0);
        
        HRCy.at<double>(1,0) = 0.0;
        HRCy.at<double>(1,1) = 1.0;
        HRCy.at<double>(1,2) = 0.0;
        
        HRCy.at<double>(2,0) = -sin(deg*M_PI/180.0);
        HRCy.at<double>(2,1) = 0.0;
        HRCy.at<double>(2,2) = cos(deg*M_PI/180.0);

        HRCz.at<double>(0,0) = cos(180.0*M_PI/180.0);
        HRCz.at<double>(0,1) = -sin(180.0*M_PI/180.0);
        HRCz.at<double>(0,2) = 0.0;
        
        HRCz.at<double>(1,0) = sin(180.0*M_PI/180.0);
        HRCz.at<double>(1,1) = cos(180.0*M_PI/180.0);
        HRCz.at<double>(1,2) = 0.0;
        
        HRCz.at<double>(2,0) = 0.0;
        HRCz.at<double>(2,1) = 0.0;
        HRCz.at<double>(2,2) = 1.0;

        WRC = WRH * HRCy *HRCz;
        CRW = WRC.inv();
        returnq = MatrixToQuaternion(CRW);
    ...
    geometry_msgs::Quaternion move_rotation2(geometry_msgs::Pose pose, double deg){
        ...
        cv::Mat WRH = cv::Mat::zeros(3,3,CV_64FC1);
        cv::Mat HRCy = cv::Mat::zeros(3,3,CV_64FC1);
        cv::Mat HRCy2 = cv::Mat::zeros(3,3,CV_64FC1);
        cv::Mat HRCz = cv::Mat::zeros(3,3,CV_64FC1);
        cv::Mat WRC = cv::Mat::zeros(3,3,CV_64FC1);
        cv::Mat CRW = cv::Mat::zeros(3,3,CV_64FC1);
        ...
        HRCy.at<double>(0,0) = cos(90.0*M_PI/180.0);
        HRCy.at<double>(0,1) = 0.0;
        HRCy.at<double>(0,2) = sin(90.0*M_PI/180.0);
        
        HRCy.at<double>(1,0) = 0.0;
        HRCy.at<double>(1,1) = 1.0;
        HRCy.at<double>(1,2) = 0.0;
        
        HRCy.at<double>(2,0) = -sin(90.0*M_PI/180.0);
        HRCy.at<double>(2,1) = 0.0;
        HRCy.at<double>(2,2) = cos(90.0*M_PI/180.0);
        ...
        HRCy2.at<double>(0,0) = cos(deg*M_PI/180.0);
        HRCy2.at<double>(0,1) = 0.0;
        HRCy2.at<double>(0,2) = sin(deg*M_PI/180.0);
        
        HRCy2.at<double>(1,0) = 0.0;
        HRCy2.at<double>(1,1) = 1.0;
        HRCy2.at<double>(1,2) = 0.0;
        
        HRCy2.at<double>(2,0) = -sin(deg*M_PI/180.0);
        HRCy2.at<double>(2,1) = 0.0;
        HRCy2.at<double>(2,2) = cos(deg*M_PI/180.0);
        WRC = WRH * HRCy * HRCz * HRCy2;
    ...
    geometry_msgs::Point move_position(geometry_msgs::Pose pose, double x, double y, double z){
        ...
        double mx1 = 1.0f - 2.0f * q.y * q.y - 2.0f * q.z * q.z;
        double mx2 = 2.0f * q.x * q.y + 2.0f * q.w * q.z;
        double mx3 = 2.0f * q.x * q.z - 2.0f * q.w * q.y;
        point.x = mx1*x + mx2*y + mx3*z + pose.position.x;
        
        double my1 = 2.0f * q.x * q.y - 2.0f * q.w * q.z;
        double my2 = 1.0f - 2.0f * q.x * q.x - 2.0f * q.z * q.z;
        double my3 = 2.0f * q.y * q.z + 2.0f * q.w * q.x;
        point.y = my1*x + my2*y + my3*z + pose.position.y;
        
        double mz1 = 2.0f * q.x * q.z + 2.0f * q.w * q.y;
        double mz2 = 2.0f * q.y * q.z - 2.0f * q.w * q.x;
        double mz3 = 1.0f - 2.0f * q.x * q.x - 2.0f * q.y * q.y;
        point.z = mz1*x + mz2*y + mz3*z + pose.position.z;
        return point;
    }
