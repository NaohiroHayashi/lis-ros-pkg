...
void target_pose2end_pose(const lis_msgs::End_poses& tp){
  ...
  while (i < 5) {//0.2sx10ループ＝2秒間座標を送信し続ける
    ...
    br.sendTransform(tf::StampedTransform(tf::Transform(tf::Quaternion(0.0,0.0,0.0,1.0), tf::Vector3(0.090, 0.000, -0.115)), now, "left_target_pose", "left_target_wrist"));
    br.sendTransform(tf::StampedTransform(tf::Transform(tf::Quaternion(0.0,0.0,0.0,1.0), tf::Vector3(0.090, 0.000, -0.115)), now, "right_target_pose", "right_target_wrist"));
    br.sendTransform(tf::StampedTransform(tf::Transform(tf::Quaternion(0.0,0.0,0.0,1.0), tf::Vector3(0.090, 0.000, -0.115)), now, "left_target_pose", "left_target_handend"));
    br.sendTransform(tf::StampedTransform(tf::Transform(tf::Quaternion(0.0,0.0,0.0,1.0), tf::Vector3(0.090, 0.000, -0.115)), now, "right_target_pose", "right_target_handend"));
    rate.sleep();
    i++;
  }
...
int main(int argc, char **argv)
{
  ...
  pub_end_pose_ = n.advertise<lis_msgs::End_poses>("end_pose_topic", 10);
  sub_target_pose_ = n.subscribe("hand_path_topic", 10, &target_pose2end_pose);
